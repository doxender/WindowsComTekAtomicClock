...
| **v2.2 → v2.3 changes** | CaptJohn — real hour/minute hand opacity in the Hora Chapín ON branch raised from 7.5% to 90% (later corrected to 10% in v1.1.3 — Dan meant "transparency 90%", not "opacity 90%"). At 7.5% the hands were unreadable; the v1.1.2 90% pass made them too prominent. See §10 Theme #7; CHANGELOG.md `[1.1.2]`. |
...
| **Minute ticks** | none |
| **Numerals** | none normally. **"12"** (Cinzel-Variable Bold 38 px, bordeaux `#7B1616`) at top (Cx, Cy−130) flashes ONLY during the noon window. **"5"** at 5 o'clock (`Cx + 130·sin(150°)`, `Cy − 130·cos(150°)`) flashes ONLY during the 5 PM window. (v1.1.3 — earlier the "12" also flashed during the 5 PM window; corrected to per-window-only.) |
| **Real hour hand** | `MakeHand(overhang 14, length 66)`, dark bordeaux `#4A0F0F`, thickness 9, round-cap. State A (flash window): flashes 1.0 / 0 with the rest of the trio. State B (Hora Chapín ON, normal): **0.1** opacity (= 90% transparent — Dan's "bring up the transparency to 90%" v1.1.3 directive; was 0.075 in v1.1.0/1.1.1, then mistakenly 0.9 in v1.1.2). State C (Hora Chapín OFF, normal): 1.0 always. |
| **Real minute hand** | `MakeHand(overhang 18, length 152)`, mid bordeaux `#641414`, thickness 5, round-cap. Same State A / B / C opacity rules as the hour hand. |
| **Real second hand** | `MakeHand(overhang 22, length 138)`, mid bordeaux `#641414`, thickness 2, round-cap. State A: flashes with the trio. State B (Hora Chapín ON, normal): 0 (hidden). State C (Hora Chapín OFF, normal): 1.0 always. |
| **"Lazy" jitter minute hand** | Line, overhang 18 / length 152, ink black `#1A1A1A`, thickness 7, round-cap. Rendered on top of the real minute hand. Visible only in state B (Hora Chapín ON, outside flash window); hidden in states A and C. Initial seed (v1.1.4): the **current local minute** when `BuildCaptJohn` runs — so the jitter hand starts visually aligned with real time. Walk: ±3 min per real-minute tick. Sync (v1.1.5): on every `:00` AND every `:30` (`local.Minute == 0 || local.Minute == 30`), the displayed minute snaps to the actual minute (0 or 30); other minute ticks just do a normal random walk step. Drift between syncs is capped at ~ ±15 min. The walk runs at all times so the displayed minute stays roughly current; only visibility is gated by state. |
...
| **State machine (v1.1.3)** | Three explicit states — **A: flash window** (real-time 11:55–12:05 OR 16:55–17:05 OR demo pinned): hour + minute + second + relevant numeral flash on/off together at 5 s / 5 s; jitter hand suppressed. **B: Hora Chapín ON, normal**: jitter hand at 100%, real hour + minute at 0.1 opacity, second hand hidden, no numerals. **C: Hora Chapín OFF, normal**: real hour + minute + second at 1.0, jitter hidden, no numerals. |
| **Default Hora Chapín state** | **OFF** (v1.1.1+) — state C is the default presentation. Hora Chapín is opt-in novelty. Persisted per-tab in `TabSettings.CaptJohnHoraChapin`. |
| **Jolly Roger overlay (v1.1.1+)** | ☠ button (44×44 with 36 px glyph) in the bottom-left of the 400×400 logical canvas. Visibility bound to `TabViewModel.IsCaptJohnTheme` via `BooleanToVisibilityConverter` so it only renders on this theme. Click opens a `ContextMenu` (placement above the button) with three checkable items: **Hora Chapín** (`StaysOpenOnClick`, two-way to `CaptJohnHoraChapin`), **Almuerzo (12:00 demo)** (`StaysOpenOnClick`), **Fini (5:00 PM demo)** (`StaysOpenOnClick`). All three are checkable; the two demo items are radio-style mutexed via `IsAlmuerzoActive` / `IsFiniActive` setters routing through a single `CaptJohnDemoMode` string on `TabViewModel`. Hosted on both `MainWindow` (per-tab) and `FloatingClockWindow`. |
| **Demo persistence** | Demos persist across `ContextMenu` close. The radio stays checked across the entire 10-minute play-out **and afterwards**, until the user explicitly toggles it off (or picks the other demo). Switching off CaptJohn theme still clears any active demo defensively (the Jolly Roger button hides on other themes; cleared in `TabViewModel.Theme` setter). |
| **Demo time mapping (v1.1.4)** | When `CaptJohnDemoMode` is "Almuerzo" or "Fini", `_digitalUpdater` records a `_captJohnDemoStartLocal` checkpoint on the first tick after the radio activates. Each subsequent tick computes `local = demoBase + (real_local − checkpoint)`, where `demoBase` is today at 11:55:00 (Almuerzo) or 16:55:00 (Fini). This makes the demo PLAY OUT at real speed: the clock instantly reads 11:55:00 on click; one real minute later it reads 11:56:00; … five real minutes later it reads 12:00:00; ten real minutes later it reads 12:05:00 and the flash window closes naturally. The demo's analog hand angles (`_hourRotate.Angle` / `_minuteRotate.Angle` / `_secondRotate.Angle`) are also re-driven from demo time so the hands track the play-out. Toggling the demo off then on, or switching demos, resets the checkpoint via the `OnCaptJohnDemoModeChanged` DP callback so a fresh play-out begins. |
