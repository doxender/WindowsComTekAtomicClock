...
| **Demo persistence (v1.1.3)** | Demos persist across `ContextMenu` close until the user explicitly toggles the radio off (or picks the other demo). Earlier `Closed`-handler clearing was per Dan's "demonstrate the visual effects until the demo button is clicked off" spec — removed. Switching off CaptJohn theme still clears any active demo defensively (the Jolly Roger button hides on other themes; cleared in `TabViewModel.Theme` setter). |
